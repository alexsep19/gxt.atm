 ...
 */
public class ShuListFilter<M, V> extends Filter<M, V> {

    private ShuListMenu<M,V> listMenu;
   ...
   */
  public ShuListFilter(ValueProvider<? super M, V> valueProvider, ListStore<V> store) {
    super(valueProvider);
    this.store = store;
//    listMenu = new ShuListMenu<M, V>(this, store);
    listMenu = createListMenu(store);
    menu = listMenu;
    }
  
  protected ShuListMenu<M, V> createListMenu(ListStore<V> store){
      return new ShuListMenu<M, V>(this, store);
  }
